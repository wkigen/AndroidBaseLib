...
public class RefreshLayout extends LinearLayout {
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (interceptCallback != null && (mCurrentRefreshStatus != RefreshStatus.REFRESHING))
                    interceptCallback.onIntercept(false);
                break;
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (handleActionMove(event)) {
                        if (interceptCallback != null) interceptCallback.onIntercept(true);
                        return true;
                    }
                    break;
                case MotionEvent.ACTION_CANCEL:
    ...
    private boolean handleActionUpOrCancel(MotionEvent event) {
        ...
        if (shouldHandleLoadingMore() && diffY <= moveDiff) {
     ...
     * 设置滑动灵敏度
     */
    public void setMoveDiff(int diff){
        this.moveDiff = diff;
     ...
     * 事件是否正在被本View拦截
     */
    public void setInterceptCallback(OnIntercept interceptCallback) {
